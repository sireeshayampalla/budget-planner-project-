...
export class BudgetController {
    ...
    static async createBudget(req, res, next) {
        try {
            ...
            const budget = await Budget.findOneAndUpdate({ userId, category, month: parsedMonth, year: parsedYear }, { limit }, { new: true, upsert: true, runValidators: true });
     ...
     */
    static async getBudgetVSExpense(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
